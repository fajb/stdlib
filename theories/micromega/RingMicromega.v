...
#[local] Abbreviation cneqb := (cneqb ceqb).
#[local] Abbreviation cltb := (cltb ceqb cleb).
...
#[local] Abbreviation NFormula := (NFormula C).
...
#[local] Abbreviation Psatz := (Psatz C).
...
#[local] Abbreviation pexpr_times_nformula := (pexpr_times_nformula cO cI cplus ctimes ceqb).
#[local] Abbreviation nformula_times_nformula := (nformula_times_nformula cO cI cplus ctimes ceqb).
#[local] Abbreviation nformula_plus_nformula := (nformula_plus_nformula cO cplus ceqb).
#[local] Abbreviation eval_Psatz := (eval_Psatz cO cI cplus ctimes ceqb cleb).
...
#[local] Abbreviation check_inconsistent := (check_inconsistent cO ceqb cleb).
...
#[local] Abbreviation check_normalised_formulas := (check_normalised_formulas cO cI cplus ctimes ceqb cleb).
...
#[local] Abbreviation eval_op2 := (eval_op2
...
#[local] Abbreviation eval_formula := (Feval rO rI rplus rtimes rminus ropp
  pow_phi rpow isProp req (fun x y => ~ req x y) rle rlt phi (@Env.nth R)).

(* We normalize Formulas by moving terms to one side *)

#[local] Abbreviation norm := (Pol_of_PExpr cO cI cplus ctimes cminus copp ceqb).
#[local] Abbreviation psub := (Psub cO cplus cminus copp ceqb).
#[local] Abbreviation padd := (Padd cO cplus ceqb).
#[local] Abbreviation pmul := (Pmul cO cI cplus ctimes ceqb).
#[local] Abbreviation popp := (Popp copp).
#[local] Abbreviation normalise := (normalise cO cI cplus ctimes cminus copp ceqb).
...
#[local] Abbreviation xnormalise := (normalise_aux copp).
#[local] Abbreviation xnegate := (negate_aux copp).
#[local] Abbreviation cnf_of_list := (cnf_of_list cO ceqb cleb).
...
#[local] Abbreviation cnf_normalise := (cnf_normalise cO cI cplus ctimes cminus copp ceqb cleb).
#[local] Abbreviation cnf_negate := (cnf_negate cO cI cplus ctimes cminus copp ceqb cleb).
...
End Micromega.
Abbreviation norm := Pol_of_PExpr (only parsing).
Abbreviation psub := Psub (only parsing).
Abbreviation padd := Padd (only parsing).
Abbreviation pmul := Pmul (only parsing).
Abbreviation popp := Popp (only parsing).

Abbreviation eval_formula :=
