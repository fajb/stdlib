...
#[local] Abbreviation eIFF := (eIFF eqb).
Abbreviation eval_f := (GFeval eqb).
...
Section S.
  ...
  #[local] Abbreviation GFormula := (@GFormula TA TX AA AF).
  ...
  Section EVAL.
    ...
    #[local] Abbreviation eval_f := (eval_f ex ea).
  ...
  End EVAL.
  #[local] Abbreviation eval_f := (eval_f ex).
...
Section S.
  ...
  #[local] Abbreviation clause := (clause Term' Annot).
  #[local] Abbreviation cnf := (cnf Term' Annot).

  Variable normalise : Term -> Annot -> cnf.
  Variable negate : Term -> Annot -> cnf.

  #[local] Abbreviation cnf_tt := (cnf_tt Term' Annot).
  #[local] Abbreviation cnf_ff := (cnf_ff Term' Annot).
  #[local] Abbreviation is_cnf_tt := (@is_cnf_tt Term' Annot).
  #[local] Abbreviation is_cnf_ff := (@is_cnf_ff Term' Annot).
  #[local] Abbreviation is_tauto :=
    (fun x y => match deduce x y with None => false | Some u => unsat u end).
  #[local] Abbreviation add_term := (add_term is_tauto).
  #[local] Abbreviation or_clause := (or_clause is_tauto).
  #[local] Abbreviation or_clause_cnf := (or_clause_cnf is_tauto).
  #[local] Abbreviation or_cnf_opt := (@or_cnf Term' Annot is_tauto).
  #[local] Abbreviation or_cnf := (@or_cnf_aux Term' Annot is_tauto).
  #[local] Abbreviation and_cnf_opt := (@and_cnf Term' Annot).

  #[local] Abbreviation TFormula TX AF := (@GFormula Term TX Annot AF).

  #[local] Abbreviation mk_and := (mk_and or_cnf_opt and_cnf_opt).
  #[local] Abbreviation mk_or := (mk_or or_cnf_opt and_cnf_opt).
  #[local] Abbreviation mk_impl := (mk_impl or_cnf_opt and_cnf_opt).
  #[local] Abbreviation mk_iff := (mk_iff or_cnf_opt and_cnf_opt).
  #[local] Abbreviation is_bool := (@is_bool Term Annot).
  #[local] Abbreviation xcnf :=
  ...
  #[local] Abbreviation cnf_checker := (cnf_checker checker).
  #[local] Abbreviation tauto_checker := (tauto_checker checker).
  ...
  #[local] Abbreviation eval_bf := (BFeval eqb).
...
End S.
Abbreviation eval_bf := (BFeval eqb).

Abbreviation tauto_checker :=
