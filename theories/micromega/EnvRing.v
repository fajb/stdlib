...
Section MakeRingPol.
 ...
 #[local] Abbreviation mk_X := (mkX cO cI).
 #[local] Abbreviation Popp := (Popp copp).
 #[local] Abbreviation PaddC := (PaddC cadd).
 #[local] Abbreviation PsubC := (PsubC csub).
 #[local] Abbreviation PaddI := (PaddI cadd).
 #[local] Abbreviation PaddX := (PaddX cO ceqb).
 #[local] Abbreviation Padd := (Padd cO cadd ceqb).
 #[local] Abbreviation PsubI := (PsubI cadd copp).
 #[local] Abbreviation PsubX := (PsubX cO copp ceqb).
 ...
 #[local] Abbreviation PmulI := (PmulI cO cI cmul ceqb).
 #[local] Abbreviation Pmul := (Pmul cO cI cadd cmul ceqb).
 #[local] Abbreviation Psquare := (Psquare cO cI cadd cmul ceqb).
...
Qed.
 ...
 #[local] Abbreviation PEeval := (ring_eval.PEeval
...
Abbreviation PEeval := (fun rO rI add mul sub opp phi pow_phi pow =>
  ring_eval.PEeval rO rI add mul sub opp pow_phi pow phi (@Env.nth _)).
